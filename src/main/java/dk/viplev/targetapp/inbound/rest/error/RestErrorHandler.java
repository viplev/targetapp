package dk.viplev.targetapp.inbound.rest.error;

import dk.viplev.targetapp.adapter.inbound.rest.dto.ErrorDTO;
import dk.viplev.targetapp.domain.exception.BadRequestException;
import dk.viplev.targetapp.domain.exception.ConflictException;
import dk.viplev.targetapp.domain.exception.ForbiddenException;
import dk.viplev.targetapp.domain.exception.NotFoundException;
import dk.viplev.targetapp.domain.exception.UnauthorizedException;
...
public class RestErrorHandler {
	...
	public ResponseEntity<ErrorDTO> handleConflict(ConflictException ex, HttpServletRequest request) {
		...
		ErrorDTO error = new ErrorDTO()
			.status(409)
			.type(CONFLICT_TYPE_URI)
			.title("Conflict - " + ex.getMessage())
			.detail(ex.getDescription())
			.instance(URI.create(request.getRequestURI()));
